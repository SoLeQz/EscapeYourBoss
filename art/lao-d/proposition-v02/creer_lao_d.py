...
for side in (-1,1):
 ...
 for row in range(arc_rows):
  t=row/(arc_rows-1);theta=2.83-(2.83+.91)*t
 ...
 def attache(reference,center):
  ...
  return end
 back=attache(surface[0],(side*.09,1.276,-.168))
 ...
 for k in range(1,19):
  t=k/18;angle=base_angle+(pi-base_angle)*t;row=[]
 ...
 rows_strap.append(attache(rows_strap[-1],(side*.085,1.052,-.193)))
