...
def matiere(nom):
 g=Graphe(nom)
 if nom=='bois':
  grain=g.noise(.9,11,3);pores=g.noise(2.5,85,2);broad=g.noise(.6,1.8,2)
 ...
 if nom=='textile':
  ...
  h=g.math('ADD',g.math('MULTIPLY',micro,.55),g.math('MULTIPLY',chiné,.45))
  col=g.ramp(h,[(.22,'88998C'),(.5,'BAC2B7'),(.8,'DFE2D4')],'Laine chinée neutre')
