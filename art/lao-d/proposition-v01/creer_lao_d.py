...
p=argparse.ArgumentParser();p.add_argument('--contrat',required=True);p.add_argument('--out',required=True)
...
for s in (-1,1):
 centers=[(s*.089,1.2,-.167),(s*.126,1.388,-.069),(s*.131,1.426,.002),(s*.126,1.388,.075),(s*.118,1.26,.103),(s*.117,1.14,.093),(s*.136,1.045,-.021)]
 vs=[]
 for x,y,z in centers:vs.extend([(x-.012,y,z),(x+.012,y,z)])
 fs=[(i*2,i*2+1,i*2+3,i*2+2) for i in range(len(centers)-1)]
 o=mesh('Bretelle textile',vs,fs,dark,'buste',1)
 for v in o.data.vertices:
  if -v.co.y>.025 and v.co.z<1.415:
   v.co.y=-max(-v.co.y,thoraxz(v.co.x,v.co.z)+.015)
 activate(o);mod=o.modifiers.new('Épaisseur sangle','SOLIDIFY');mod.thickness=.004;o.modifiers.move(len(o.modifiers)-1,0);bpy.ops.object.modifier_apply(modifier=mod.name)
...
params=dict(filepath=str(out/'lao-d-v01.glb'),export_format='GLB',use_selection=True,export_yup=True,export_animations=False,export_skins=True,export_morph=True,export_cameras=False,export_lights=False,export_def_bones=False,export_leaf_bone=False,export_apply=False)
...
bpy.ops.wm.save_as_mainfile(filepath=str(out/'lao-d-v01.blend'));bpy.ops.render.render(write_still=True)
triangles=0
for o in objects:o.data.calc_loop_triangles();triangles+=len(o.data.loop_triangles)
rapport={'versionBlender':bpy.app.version_string,'jalon':'B proposition à valider','objetsSkinnes':len(objects),'triangles':triangles,'materiaux':len(objects),'glbOctets':(out/'lao-d-v01.glb').stat().st_size,'textures':0,'expressions':'regard, clignements, doigts indépendants non finalisés — jalon C','source':'géométrie originale créée dans Blender, aucun asset tiers'}
