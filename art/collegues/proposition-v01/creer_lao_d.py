...
p=argparse.ArgumentParser();p.add_argument('--contrat',required=True);p.add_argument('--out',required=True);p.add_argument('--nom',default='lao-d-v01');p.add_argument('--profil',choices=['lao-d','directeur','zhang','liu'],default='lao-d')
a=p.parse_args(sys.argv[sys.argv.index('--')+1:]);out=Path(a.out)
prefixe=('lao-d' if a.profil=='lao-d' else a.profil)+'-v'
if not a.nom.startswith(prefixe) or not a.nom[len(prefixe):].isdigit():raise RuntimeError('Nom attendu : '+prefixe+'NN')
...
def mesh(name,verts,faces,material,weights,sub=0):
 ...
 for i,v in enumerate(verts):
  w=weights[i] if isinstance(weights,list) else {weights:1} if isinstance(weights,str) else weights(v)
...
for sign,c in [(1,4),(-1,12)]:
 ...
 for y,cx,rx,rz in [(1.28,.244,.067,.078),(1.23,.25,.061,.073),(1.17,.254,.058,.065),(1.13,.253,.057,.065),(1.10,.249,.055,.06),(1.06,.245,.049,.055),(.96,.237,.046,.05),(.881,.233,.04,.043),(.875,.233,.04,.043)]:
  ...
  prev=new
# Les manches des collègues sont identifiées par leur topologie avant subdivision.
# Une sélection par X seul laisse le bord intérieur du poignet attaché au buste.
vestweights=bodyweights
if a.profil!='lao-d':
 vestweights=[]
 for i,(x,y,z) in enumerate(vs):
  suffix='R' if x>0 else 'L'
  if i<len(rows)*n:
   shoulder=.38*smooth((abs(x)-.16)/.055)*smooth((y-1.23)/.11)
  else:
   shoulder=1-.25*smooth((y-1.23)/.05)*smooth((.25-abs(x))/.07)
  fore=smooth((1.18-y)/.14)
  vestweights.append({'buste':1-shoulder,'epaule'+suffix:shoulder*(1-fore),'coude'+suffix:shoulder*fore})
j=mesh('Veste continue — emmanchures cousues',vs,fs,jacket,vestweights,1)
def thoraxz(x,y):
...
profil_info=None
if a.profil!='lao-d':
 # Module local livré avec la source, sans dépendance externe.
 sys.path.insert(0,str(Path(__file__).resolve().parent))
 from variantes_bureau import appliquer
 profil_info=appliquer(a.profil,globals())

# Rassembler par matière avec une armature partagée. Source éditable par îlots,
# poids et UV conservés ; nombre de surfaces mesuré dans le rapport final.
...
rapport={'versionBlender':bpy.app.version_string,'jalon':'B proposition à valider','objetsSkinnes':len(objects),'triangles':triangles,'materiaux':len(objects),'glbOctets':(out/(a.nom+'.glb')).stat().st_size,'textures':0,'profil':profil_info,'bretelles':strap_report,'expressions':'regard, clignements, doigts indépendants non finalisés — jalon C','source':'géométrie originale créée dans Blender, aucun asset tiers'}
